...
- **Poisson clock:** correct time indexing (no observation silently excluded) and simulation of the residual interval. The arrival times now realise the paper's intensity `lambda(t) = lambda0 (1 + t)` (the inverse cumulative intensity had a missing factor 2, silently doubling `lambda0` relative to the paper); numerical results at a given `lambda0` change accordingly.
- **Self-loop distances:** correct geodesic on loop edges (no spurious 0.0), across all distance kernels (now deduplicated).
- **Distance caches** are invalidated on graph edits, and a re-run `precomputing()` actually recomputes. Invalidation now also covers the bulk networkx mutators (`add_edges_from`, `add_nodes_from`, `remove_edges_from`, `remove_nodes_from`, `update`, `clear`, `clear_edges`), which bypass the per-edge overrides, so a stale cache is never served after a bulk edit. Edge lengths carried by incoming graph data are validated at construction — a degenerate edge (e.g. a zero-length edge from a collapsed epsilon-net) fails with a clear error even when `precompute=False`.
- The `node_measure` energy raises instead of silently returning 0.0 when no node carries a positive measure; its kernel carries `obs_weight` as a float measure, so fractional population weights are no longer truncated to zero.
- **Reproducible RNG throughout:** a `random_state` is normalized to a Generator **once** and reused, so successive `sample()` calls on one strategy draw different points and the SA-based Fréchet mean decorrelates across clusters and Lloyd iterations (an int seed used to restart the same stream on every call). `RandomInit` samples **without** replacement (distinct centers; raises if `k` exceeds the number of observations); `run_parallel` draws distinct auto-seeds and breaks exact energy ties by seed, so the returned best run no longer depends on completion order; the graph layout seeds `spring_layout`.
...
- **Metric-correct Brownian motion** on manifolds via `random_tangent` (sphere and Bolza; unsupported manifolds raise). The step is now exactly `exp(x, sqrt(t) V)` with `V` a standard Gaussian tangent vector; the previous extra scalar factor made each step a product of two normals (heavier-tailed, wrong radial law at finite `step_size`), inconsistent with the single-Gaussian graph step.
- Paper protocol: equal restart budgets across methods, honest grid-timing attribution, and a harmonised population-measure selection criterion. SA-sphere now selects its best run on the same fixed measure (the full data) as CLVQ and k-medoids, instead of each run's own bootstrap resample (a noisy criterion); the overhead experiment's SA-graph runs get their own entropy instead of duplicating the sphere campaign's; every experiment's dataset stream is derived from structured entropy (no cross-experiment collisions); and the tracked seed, whose convergence diagnostic inflates its SA time, is excluded from the timing aggregation.
...
- Graph generators: second-level node names no longer collide for `n_a >= 11`; `complete_quantum_graph` requires strictly-positive, symmetric similarities (they become edge lengths); the `generate_simple_random_graph` edge weight is the correct harmonic mean; the unused per-edge `distribution` attribute (and the internal `UniformDistribution` class) are removed.
