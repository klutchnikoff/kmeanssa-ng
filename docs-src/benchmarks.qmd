...
For the latest performance results and analysis, please see the [Performance page](performance.md).

This document provides technical instructions for developers on how to run, manage, and contribute to the performance benchmarks for `kmeanssa-ng`.
...
Benchmarks are not run in CI by default due to performance variability on shared runners. To enable them, you can add a job like the following to `.gitlab-ci.yml`:
...
4.  Ensure the new benchmark is covered by the documentation if it represents a new feature.

For more details, refer to the [pytest-benchmark documentation](https://pytest-benchmark.readthedocs.io/).
