...
`kmeanssa-ng` clusters data that lives on complex network structures (**quantum graphs**), on **Riemannian manifolds** such as spheres and hyperbolic space, or on any metric space where standard Euclidean distance does not apply. Manifolds can be clustered directly, or approximated by a graph (an $\varepsilon$-net) and clustered on that. It uses a simulated annealing approach for robust convergence.
