...
To cluster data on a quantum graph, you need observations—`QGPoint` objects. The `sample_points(n)` method generates them uniformly across the graph:
...
The sampling is **proportional to edge lengths**: a 5 km road gets more points than a 1 km road, ensuring true uniformity across the graph's total length. 
...
**Precomputing is mandatory**: Always call `precomputing()` after creating or modifying a graph. This caches shortest paths and is required for distance computation. Generators do this automatically.
