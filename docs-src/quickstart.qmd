...
This guide provides a basic example of how to use `kmeanssa-ng` to perform clustering on a quantum graph and visualize the results.
...
Next, we sample data points uniformly across the graph. These are the points we want to cluster.
...
Finally, we visualize the graph, the data points, and the resulting cluster centers using the built-in plotting capabilities of `kmeanssa-ng`. Note that you need to install the `plot` extras for this: `pip install kmeanssa-ng[plot]`.
