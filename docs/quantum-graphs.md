...
`(edge, position)`, where `position` is the **distance from the first
node of the edge**, so it ranges over `[0, length]` (an absolute
distance, not a fraction of the edge). The distance between any two
points is the **geodesic distance**—the length of the shortest path
through the graph—making this a genuine metric space suitable for
k-means clustering.
...
immutable and specified by an `(edge, position)` pair. For example, on
an edge of length 1, `QGPoint(graph, edge=(0,1), position=0.3)` places a
point at distance 0.3 from node 0 toward node 1 (on an edge of length 5,
that same `position=0.3` would sit near node 0). These are the data you
want to cluster.
...
**Same edge**: The distance is simply $|\text{pos}_1 - \text{pos}_2|$
(positions are absolute distances). If two delivery points are at 1 km
and 4 km along a 5 km road, they’re $|4 - 1| = 3$ km apart.
...
The `UniformNodeSampling` strategy samples **uniformly over the graph
nodes**: every node is equally likely, and node weights are ignored.
(For continuous uniformity along the edges—so a 5 km road gets more
points than a 1 km road—use `UniformEdgeSampling`; to weight by node
importance, use `WeightedNodeSampling`, shown next.)
...
```
    ...
    Observations per node (approximate):
      Node A (weight=1.0): 124 observations
      Node B (weight=5.0): 734 observations
...
```
    ...
    labelled 40 nodes; energy = 62.905
