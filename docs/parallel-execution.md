...
best_centers = run_parallel(points, k=2, n_runs=10)
print(f"Best result has {len(best_centers)} centers")
```

**Parameters**:

- `points`: List of observations to cluster
...
result1 = run_parallel(points, k=2, n_runs=5, seeds=seeds)
result2 = run_parallel(points, k=2, n_runs=5, seeds=seeds)

# Results will be very similar (within numerical precision)
```
