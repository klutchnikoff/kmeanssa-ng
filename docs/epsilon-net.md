...
`kmeanssa-ng` clusters points on a [quantum graph](quantum-graphs.md)—a
...
The bridge is an **$\varepsilon$-net**: a finite set of points spread so
densely over the manifold that no location is farther than $\varepsilon$
from some net point. Connect the net points that are close together and
you obtain a quantum graph that **approximates the manifold**—shortest
paths through the graph track true geodesics, to within $\varepsilon$.
You can then cluster on that graph exactly as on any other. This
approximation is the computational backbone of online $k$-means on
geodesic spaces: as the net is refined ($\varepsilon \to 0$), clustering
on the graph converges to clustering on the manifold.

The manifolds themselves (`create_sphere`, `create_hyperbolic_space`,
`RiemannianPoint`) are described in [Riemannian
Manifolds](riemannian-manifolds.md); this page is about turning one into
a graph.
...
Because the mesh **is** a quantum graph, clustering is the familiar
workflow:
...
How well the graph approximates the manifold depends on how **evenly**
the net covers it. Placement follows the same strategy pattern as
[sampling](quantum-graphs.md) and
[initialization](simulated-annealing.md):
...
> [!NOTE]
>
> On **non-compact** domains such as hyperbolic space, unbounded
> repulsion pushes points toward the boundary and leaves the interior
> sparse. `RepulsionNet` targets **compact** manifolds; bounded regions
> with a soft confining potential are a planned extension.
