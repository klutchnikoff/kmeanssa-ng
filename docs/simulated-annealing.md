...
$$\alpha(t) = \min(h \cdot \beta_0 \cdot \log(1 + t), 1)$$ where $h$ is
the time interval. This controls the strength of attraction toward the
nearest observation.
...
**Mathematical role**: Euler discretization step $\Delta t$ for solving
the stochastic differential equation (SDE). Smaller values provide more
accurate simulation of the continuous process.
