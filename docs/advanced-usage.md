...
observations = my_space.sample_points(
    200, strategy=UniformNodeSampling(random_state=0)
)
k = 4

# Create the predefined centers for our strategy
points_for_centers = my_space.sample_points(
    k, strategy=UniformNodeSampling(random_state=1)
)
...
# Run the algorithm with your strategy (robustification defaults to MinimizeEnergy)
sa = SimulatedAnnealing(observations, k=k)
final_centers = sa.run(initialization_strategy=my_strategy)
