...
class TestQuantumGraphDiameter:
    ...
    def test_diameter_calculation(self):
        ...
        assert diameter == 3.0  # 0 -> 1 -> 2 (2 + 3 = 5) vs 0 -> 2 -> 1 (1 + 3 = 4) vs direct paths
...
class TestPrecomputingConnectivity:
    ...
    def test_precomputing_disconnected_graph(self):
        ...
        with pytest.raises(ValueError, match="Graph must be connected.*Found 2 connected components"):
            graph.precomputing()
...
class TestSamplingEdgeCases:
    ...
    def test_sample_point_invalid_where(self):
        ...
        with pytest.raises(ValueError, match='The parameter "where" must be either "Node" or "Edge"'):
    ...
    def test_sample_point_node_no_weights(self):
        ...
        with pytest.raises(NotImplementedError, match="Node sampling requires 'weight' attribute"):
    ...
    def test_sample_point_edge_no_attributes(self):
        ...
        with pytest.raises(NotImplementedError, match="Edge sampling requires 'weight' and 'distribution'"):
    ...
    def test_sample_kpp_centers_no_precomputing(self):
        ...
        with pytest.raises(ValueError, match="Must call precomputing\\(\\) before sample_kpp_centers"):
